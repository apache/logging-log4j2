...
public class LoggerContextAwarePostProcessor implements InstancePostProcessor {
    private final Supplier<? extends LoggerContext> loggerContextSupplier;

    public LoggerContextAwarePostProcessor(final Supplier<? extends LoggerContext> loggerContextSupplier) {
    ...
    public <T> T postProcessBeforeInitialization(final ResolvableKey<T> resolvableKey, final T instance) {
        if (instance instanceof LoggerContextAware) {
            ((LoggerContextAware) instance).setLoggerContext(loggerContextSupplier.get());
        }
        return instance;
    }
}
