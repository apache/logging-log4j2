...
public class AsyncQueueFullPolicyFactory {
    ...
    public static AsyncQueueFullPolicy create() {
        return create(PropertyEnvironment.getGlobal().getProperty(CoreKeys.AsyncQueueFullPolicy.class));
    }

    static AsyncQueueFullPolicy create(final CoreKeys.AsyncQueueFullPolicy config) {
        final String router = config.className();
        if (router == null
                || isRouterSelected(
        ...
        if (isRouterSelected(
            ...
            return createDiscardingAsyncQueueFullPolicy(config);
        }
    ...
    private static AsyncQueueFullPolicy createDiscardingAsyncQueueFullPolicy(
        ...
        final Level thresholdLevel = config.discardThreshold();
