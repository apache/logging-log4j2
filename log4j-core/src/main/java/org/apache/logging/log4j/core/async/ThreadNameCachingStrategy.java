...
public enum ThreadNameCachingStrategy { // LOG4J2-467
    ...
    public static ThreadNameCachingStrategy create() {
        final String name =
                PropertiesUtil.getProperties().getStringProperty(Log4jPropertyKey.ASYNC_LOGGER_THREAD_NAME_STRATEGY);
        try {
            final ThreadNameCachingStrategy result =
                    name != null ? ThreadNameCachingStrategy.valueOf(name) : DEFAULT_STRATEGY;
            LOGGER.debug(
                    "{}={} (user specified {}, default is {})",
                    Log4jPropertyKey.ASYNC_LOGGER_THREAD_NAME_STRATEGY,
                    result.name(),
                    name,
