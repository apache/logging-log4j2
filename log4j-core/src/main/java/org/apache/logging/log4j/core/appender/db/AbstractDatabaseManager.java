...
public abstract class AbstractDatabaseManager extends AbstractManager implements Flushable {
     ...
     */
    public final synchronized void write(final LogEvent event, final Object serializable) {
        if (isBuffered()) {
            buffer(event);
        } else {
     ...
     * @param event The event to write to the database.
     */
    protected abstract void writeInternal(LogEvent event, Object serializable);

    protected void writeThrough(final LogEvent event, final Object serializable) {
        this.connectAndStart();
        try {
