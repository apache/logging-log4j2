...
public class ReconfigureAppenderTest {
    ...
    private void removeManagerUsingReflection() {
        try {
            ...
            Map<String, AbstractManager> map = (Map<String, AbstractManager>) field.get(null);
    ...
    private void createAndAddAppender() {
        ConfigurationBuilder<BuiltConfiguration> config_builder = ConfigurationBuilderFactory.newConfigurationBuilder();
        ...
        LoggerContext logger_context = Configurator.initialize(config_builder.build());

        // Retrieve the logger.
        Logger logger = (Logger) LogManager.getLogger(this.getClass());

        Builder pattern_builder = PatternLayout.newBuilder().withPattern("[%d{dd-MM-yy HH:mm:ss}] %p %m %throwable %n");
