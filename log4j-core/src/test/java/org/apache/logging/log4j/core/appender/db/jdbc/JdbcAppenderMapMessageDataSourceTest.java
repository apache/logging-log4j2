...
public class JdbcAppenderMapMessageDataSourceTest extends AbstractJdbcDataSourceTest {
    ...
    public void testDataSourceConfig() throws Exception {
        final Connection connection = jdbcRule.getConnectionSource().getConnection();
        try {
            ...
            mapMessage.put("Id", "1");
            mapMessage.put("ColumnA", "ValueA");
            mapMessage.put("ColumnB", "ValueB");
            logger.info(mapMessage);

            final Statement statement = connection.createStatement();
            final ResultSet resultSet = statement.executeQuery("SELECT Id, ColumnA, ColumnB FROM dsLogEntry ORDER BY Id");
            try {
    ...
    public void testTruncate() throws SQLException {
        ...
        try {
            ...
            final MapMessage mapMessage = new MapMessage();
            mapMessage.put("Id", "1");
            mapMessage.put("ColumnA", StringUtils.repeat('A', 1000));
            mapMessage.put("ColumnB", StringUtils.repeat('B', 1000));
            logger.info(mapMessage);
            final Statement statement = connection.createStatement();
            final ResultSet resultSet = statement.executeQuery("SELECT Id, ColumnA, ColumnB FROM dsLogEntry ORDER BY Id");
            try {
