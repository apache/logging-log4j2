...
class PropertiesConfigurationTest {
    ...
    void testRootLoggerOnly(final Configuration config) {
        ...
        final Map<String, LoggerConfig> loggers = config.getLoggers();
        assertThat(loggers).isNotNull();
        assertThat(loggers).describedAs("Incorrect number of LoggerConfigs: " + loggers.size()).hasSize(1);
        final Filter filter = config.getFilter();
        assertThat(filter).describedAs("No Filter").isNotNull();
    ...
    void testRollingFile(final Configuration config) {
        ...
        final Map<String, Appender> appenders = config.getAppenders();
        assertThat(appenders).isNotNull();
        assertThat(appenders).describedAs("Incorrect number of Appenders: " + appenders.size()).hasSize(3);
        final Map<String, LoggerConfig> loggers = config.getLoggers();
        assertThat(loggers).isNotNull();
        assertThat(loggers).describedAs("Incorrect number of LoggerConfigs: " + loggers.size()).hasSize(2);
        final Filter filter = config.getFilter();
