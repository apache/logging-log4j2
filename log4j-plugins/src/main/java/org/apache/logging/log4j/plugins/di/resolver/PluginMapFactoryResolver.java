 ...
 */
public class PluginMapFactoryResolver extends AbstractPluginFactoryResolver {
    @Override
    ...
    public Supplier<?> getFactory(final ResolvableKey<?> resolvableKey, final InstanceFactory instanceFactory) {
        ...
        if (componentType instanceof ParameterizedType) {
            ...
            if (rawType == Supplier.class && typeArguments.length == 1) {
                final Type suppliedType = typeArguments[0];
                return () -> Plugins.streamPluginTypesMatching(instanceFactory, namespace, suppliedType)
