...
[IMPORTANT]
====
These filters are provided by **third-party** vendors and are not maintained by the
...
Sometimes xref:#CompositeFilter[composing filters sequentially] is not enough. Using a `RoutingFilter`, different filter chains can be
