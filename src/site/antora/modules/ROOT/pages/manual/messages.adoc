...
<2> Contextual fields:
`tags`:: The stack of xref:manual/thread-context.adoc#usage[nested diagnostic context] messages.
`labels`:: The key/value pairs of the current xref:manual/thread-context.adoc#usage[mapped diagnostic context].
...
* xref:manual/appenders.adoc#NoSQLAppenderMongoDB[MongoDB NoSQL provider] converts to fields in MongoDB object
