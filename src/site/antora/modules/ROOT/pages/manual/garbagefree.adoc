...
* Logging very large messages (i.e., more than <<log4j2.maxReusableMsgSize[`log4j2.maxReusableMsgSize`] characters, which defaults to 518), when all loggers are xref:manual/async.adoc,asynchronous loggers>>, will cause the internal `StringBuilder` in the
