...
Some layouts implement `LocationAware` and they determine if a location information is required according to these rules:

<<GELFLayout,GelfLayout>>::
+
The GELF layout requires location information only if the pattern of the `messagePattern` requires location information.
+
See <<location-pattern-layout,PatternLayout case>> below.

<<HTMLLayout,HtmlLayout>>::
+
The HTML layout requires location information if the <<HtmlLocationInfo,locationInfo>> configuration attribute is set to `true`.

[[location-pattern-layout]]<<PatternLayout,PatternLayout>>::
+
The pattern layout will require location information if at least one of these patterns is used:
+
* <<PatternClass,`%C` or `%class`>>,
* <<PatternFile,`%F` or `%file`>>,
* <<PatternLocation,`%l` or `%location`>>,
* <<PatternLine,`%L` or `%line`>>,
* <<PatternMethod,`%M` or `%method`>>.
