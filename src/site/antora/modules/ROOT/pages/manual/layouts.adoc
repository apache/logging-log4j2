...
While layouts are responsible for formatting a log event into a string or byte array, they do not typically modify the content of the event itself.
If you need to mask sensitive information (such as PII), inject additional metadata, or otherwise manipulate the message text before it reaches the layout, consider using a xref:manual/appenders/delegating.adoc#RewritePolicy[Rewrite Policy].
Rewrite policies allow for pre-processing of the log event content, ensuring that sensitive data is handled consistently across all configured layouts.
