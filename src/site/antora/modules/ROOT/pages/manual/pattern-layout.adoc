...
|literal text
a|Garbage-free, but care is needed for xref:manual/configuration.adoc#property-substitution[property substitution], including xref:manual/lookups.adoc[].
...
*Pattern Layout has powerful directives to inject external values, and you should always prefer those whenever possible over lookups!*
For example, always prefer <<converter-map, the `map` converter>> over xref:manual/lookups.adoc#MapLookup[the `map` lookup].
====
|===
