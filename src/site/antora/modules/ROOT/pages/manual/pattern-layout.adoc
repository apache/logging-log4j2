...
The pattern layout supports styling your text using a variety of ANSI escape sequence, which can be used through the
xref:#converter-highlight[`%highlight`] and xref:#converter-style[`%style`] pattern converters.
...
The style specifiers have the following effects (see
https://en.wikipedia.org/wiki/ANSI_escape_code#SGR_(Select_Graphic_Rendition)_parameters[Select Graphic Rendition]
for details):
