...
A locale tag in https://www.rfc-editor.org/rfc/bcp/bcp47.txt[BCP 47] format (e.g., `de-DE`, `fr-FR`) can be supplied as a second argument to control locale-sensitive formatting such as month names.
When only two arguments are given, the second argument is treated as a *timezone ID* if it matches any known `TimeZone` ID (timezone always takes precedence); otherwise it is treated as a *locale tag* if it is a valid BCP 47 language tag.
