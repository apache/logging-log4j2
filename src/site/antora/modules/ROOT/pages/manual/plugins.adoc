...
Aliases allow a plugin to be referenced by multiple names in configuration files.
For example, a plugin with `@PluginAliases({"MyApp", "MyApplication"})` can be referred to by any of those names.
Aliases are also supported for plugin attributes, allowing configuration elements to use alternative attribute names.
...
==== Plugin factory conditional annotations

Conditional annotations allow plugins to be registered only when certain conditions are met.
They are typically used on bundle classes or factory methods to control whether bindings should be created based on the runtime environment.

The following built-in conditional annotations are available:

link:../javadoc/log4j-plugins/org/apache/logging/log4j/plugins/condition/ConditionalOnMissingBinding.html[`@ConditionalOnMissingBinding`]::
Checks if a binding exists for the return type of the annotated factory method.
When no binding exists for that key, the method will be considered for binding.
This is commonly used to provide default implementations that can be overridden by user-defined plugins.

link:../javadoc/log4j-plugins/org/apache/logging/log4j/plugins/condition/ConditionalOnPresentBindings.html[`@ConditionalOnPresentBindings`]::
Only creates a binding if all the requested dependencies exist.
The required dependencies are specified via the `bindings` attribute.
This is useful for plugins that require optional dependencies to be present on the classpath.

Custom conditions can be created by implementing the `Condition` interface and annotating a custom annotation with `@Conditional`.
