...
NOTE: To configure xref:manual/status-logger.adoc[] you can also place its configuration in a file named `log4j2.StatusLogger.properties`.
...
NOTE: To configure xref:manual/simple-logger.adoc[] you can also place its configuration in a file named `log4j2.simplelog.properties`.

The xref:manual/simple-logger.adoc[] implementation contained in `log4j-api` can be configured using these properties:
...
Log4j Core supports the same properties as xref:manual/simple-logger.adoc[] for the configuration of the thread context.
