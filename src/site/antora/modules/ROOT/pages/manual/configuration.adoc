...
[NOTE]
====
To prevent a chicken-and-egg problem some configuration options (e.g., the configuration file location) can only be supplied through xref:manual/systemproperties.adoc[system properties].
====

[#configuration-file]
== Configuration file

TIP: For a quick example of a configuration file see the xref:manual/installation.adoc#impl-core-config[Configuring Log4j Core section].
...
While Log4j makes every effort to only compute it if it was requested by a configured layout (cf. xref:manual/layouts.adoc[Layouts]), this setting provides a kill switch to disable it.

[TIP]
====
This setting only applies to the computation of location at **runtime**.
