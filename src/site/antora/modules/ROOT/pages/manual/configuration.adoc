...
. If no configuration file is found, Log4j uses the link:../javadoc/log4j-core/org/apache/logging/log4j/core/config/DefaultConfiguration[DefaultConfiguration], and xref:manual/status-logger.adoc[the status logger] prints a warning.
...
WARNING: Since version `2.24.0` this attribute is deprecated and should be replaced with the
