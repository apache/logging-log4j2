...
| [[DefaultRolloverStrategy-attr-maxCompressionDelaySeconds]]maxCompressionDelaySeconds
| `int`
| `0`
|
Maximum random delay, in seconds, before compressing archived log files.

Use this attribute to spread compression workload when many applications roll over at the same time.
A value of `0` disables the delay and starts compression immediately.
|===
