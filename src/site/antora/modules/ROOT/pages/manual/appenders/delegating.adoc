...
[%collapsible]
=====
include::partial$features/queue-conversant.adoc[]
=====
+
xref:plugin-reference.adoc#org-apache-logging-log4j_log4j-conversant_org-apache-logging-log4j-conversant-DisruptorBlockingQueueFactory[{plugin-reference-marker} Plugin reference for `DisruptorBlockingQueue`]
...
The
{log4j2-url}/manual/status-logger.html[status logger]
to use to print diagnostic messages in the script.
...
You can deliver log events for different
{log4j2-url}/manual/markers.html[markers]
into separate log files, using the following configuration:
...
In the example below, a different log file is created for each
