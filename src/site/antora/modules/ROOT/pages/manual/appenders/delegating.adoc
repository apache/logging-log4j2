...
The **`https://github.com/Neel1210/log4j-utils/blob/main/src/main/java/in/org/nnm/log4j/utils/MessageRewritePolicy.java[MessageRewritePolicy]`** Provided by ( **https://github.com/Neel1210/log4j-utils[log4j-utils]** ) allows you to manipulate the text of a log message before it is processed by a downstream appender.
This is particularly useful for defining multiple regex-based replacement rules to mask sensitive information (PII) within the log message.
