...
Embedding the lookup inside a longer fixed segment, such as `logs/user-${ctx:userId}.log`, typically fails to open the file instead of escaping the directory, so the exact `fileName` pattern matters.
