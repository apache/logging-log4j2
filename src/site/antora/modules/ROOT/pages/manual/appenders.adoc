...
| Default value
| xref:manual/systemproperties.adoc#log4j.gc.encoderByteBufferSize[`log4j.gc.encoderByteBufferSize`]
|===
...
If the xref:manual/systemproperties.adoc#log4j.gc.enableDirectEncoders[`log4j.gc.enableDirectEncoders`] configuration property is set to `true`, the internal `ByteBuffer` will always be used.
...
By default, Log4j Core uses the
{log4j2-url}/manual/status-logger.html[status logger]
to report exceptions that occur in appenders.
...
{log4j2-url}/manual/status-logger.html[]
