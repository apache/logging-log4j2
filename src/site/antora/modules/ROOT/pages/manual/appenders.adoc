...
This appender is {log4j2-url}/manual/messages.html#MapMessage[`MapMessage`]-aware.
...
| String
| _Required_
| The URL of the provider to use as defined by https://download.oracle.com/javase/{java-target-version}/docs/api/javax/naming/Context.html#PROVIDER_URL[`PROVIDER_URL`]. From Log4j 2.17, only the `java:` protocol is supported.
...
| String
| null
| The name of the identity of the Principal as specified by https://download.oracle.com/javase/{java-target-version}/docs/api/javax/naming/Context.html#SECURITY_PRINCIPAL[`SECURITY_PRINCIPAL`]. If a securityPrincipalName is specified without `securityCredentials`, a warning message will be logged as this is likely to cause problems.

| securityCredentials
| String
| null
| The security credentials for the principal as specified by https://download.oracle.com/javase/{java-target-version}/docs/api/javax/naming/Context.html#SECURITY_CREDENTIALS[`SECURITY_CREDENTIALS`].
...
| String
| null
| A colon-separated list of package prefixes for the class name of the factory class that will create a URL context factory as defined by https://download.oracle.com/javase/{java-target-version}/docs/api/javax/naming/Context.html#URL_PKG_PREFIXES[`URL_PKG_PREFIXES`].
...
Any failure sending to Kafka will be reported as an error to {log4j2-url}/manual/status-logger.html[status logger] and the log event will be dropped (the ignoreExceptions parameter will not be effective).
...
|statusLogger |StatusLogger |The {log4j2-url}/manual/status-logger.html[status logger] that can be used to log
...
While annotating your appender with `@Plugin` and `@Configurable`, you need to make sure that

* The plugin name in `@Plugin::value` or the simple class name of the plugin if no custom name is given must be unique across all available `Appender` plugins.
