...
<1> Encodes events using xref:manual/pattern-layout.adoc[] and forwards the call to `ServletContext.log()`.
...
Modern application servers usually forward their calls to a proper logging framework and use a **fixed** logging level for all the events.
====
