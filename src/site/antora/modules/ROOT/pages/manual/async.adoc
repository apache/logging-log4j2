...
They are based on link:#UnderTheHood[LMAX Disruptor], a lock-free inter-thread communication library, instead of queues, resulting in higher throughput and lower latency.
+
...
xref:manual/layouts.adoc#location-information[Computing the location information (i.e., the caller class, method, file, and line number) of a log event is an expensive operation.]
