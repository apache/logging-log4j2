...
Problems may arise especially if xref:manual/appenders.adoc#rollingfileappender[rolling file appender] is used.
...
The second approach is more complex and requires changes to the configuration of the application server.
...
In order to do it you must create a `web.xml` descriptor and add the `Log4jServletContextListener` explicitly as the **first** context listener:
...
In order to use it, you need to add the following runtime dependency to your web application project:

include::partial$manual/dependencies-log4j-taglib.adoc[]
...
Currently, it only works with JavaServer Pages 2.3 or older and no version compatible with Jakarta Server Pages 3.0 is available.
====
...
A complete JSP page example is available
{antora-examples-url}/manual/webapp/taglib.jsp[on GitHub].
...
+
In order to use Log4j Core, you need to set the `add-logging-api-dependencies` attribute of the logging subsystem to `false`.
...
In order to share Log4j Core between applications, you need to share at least these two JAR files:
...
Maven::
If you use Maven, you can declare the
...
Gradle::
If you use Gradle, you can add `log4j-api` to the `providedCompile` configuration, while `log4j-core` to the `providedRuntime` configuration.
See https://docs.gradle.org/current/userguide/war_plugin.html#sec:war_dependency_management[Gradle WAR plugin] for more details.
...
When using a shared instance of Log4j Core, you might be interested to identify the application associated to a given log event.
...
. You can create a separate logger context for each web application and one for the common libraries.
...
[IMPORTANT]
====
These two approaches deliver similar results for log events generates by the web applications themselves or the libraries bundled in the WAR or EAR archive.
...
These libraries will use the logger context associated to the web application that uses them.
+
Application server implementation usually use instance `Logger` fields.

Since the first kind of libraries is more common, counterintuitively the <<single-context>> approach will usually give better results than the <<multiple-contexts>> approach.
====
...
In order to use a single logger context you need to set the
...
Following example demonstrates this scheme using a Socket Appender writing to Elasticsearch:
+
[tabs]
====
XML::
...
xref:manual/appenders.adoc#RoutingAppender[routing appender]
...
the classes of each classloader will use the logger context associated to the classloader.
...
[#replace]
== Replacing application server logging subsystem
...
as internal logging system.
Tomcat JULI uses `java.util.logging` as default logging implementation, but since Tomcat 8.5 you can easily replace it with a different backend.

In order to use Log4j Core as logging backend, you need to modify the
