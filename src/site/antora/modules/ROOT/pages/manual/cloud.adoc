...
* *For writing to an external service*, use a xref:manual/appenders.adoc#SocketAppender[Socket Appender] and make sure to set the protocol to TCP and configure the null delimiter of the associated layout.
...
A common excuse for writing logs to files is to use them as a buffer prior to ingesting them either to the logging system proxy or to the log storage system.
Even in this case, most of the time it is sign of bad architecture design, and costs extra resources and efficiency degradation.
...
Ship configuration files along with the application, and choose a subset of them during deployment::

You can ship environment-specific xref:manual/configuration.adoc[Log4j configuration files] (`log4j2-common.xml`, `log4j2-local.xml`, `log4j2-test.xml`, `log4j2-prod.xml`, etc.) and use xref:manual/systemproperties.adoc#log4j2.configurationFile[the `log4j2.configurationFile` configuration property] to selectively activate these configurations.
For instance, while running locally, use
+
[source,properties]
----
log4j2.configurationFile=log4j2-common.xml,log4j2-local.xml
----
+
while running on test environment, use
+
[source,properties]
----
log4j2.configurationFile=log4j2-common.xml,log4j2-test.xml
----
+
and so on.
+
...
See xref:log4j-docker.adoc[Log4j Docker] for Docker-specific Log4j features, e.g., xref:manual/lookups.adoc#DockerLookup[Docker Lookup].
