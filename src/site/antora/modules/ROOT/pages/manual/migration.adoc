...
has reached **End of Life** in 2015 (see
https://blogs.apache.org/foundation/entry/apache_logging_services_project_announces[announcement])
...
| xref:manual/appenders.adoc#RollingFileAppender[RollingFile]
| See <<rolling-file-conversion>>
...
| xref:manual/appenders.adoc#jms-appender[JMS]
|
...
| xref:manual/appenders.adoc#SMTPAppender[SMTP]
|
...
xref:manual/appenders.adoc#DefaultRolloverStrategy[default rollover strategy] to `min`.
+
[source,xml]
----
...
== [[Log4j1.2Bridge]]Log4j 1 to Log4j 2 bridge
