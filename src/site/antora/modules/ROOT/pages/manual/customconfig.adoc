...
<1> Passing the optional `LoggerContext` argument as null, since this `Configuration` is not associated with a `LoggerContext` yet
<2> Passing the optional configuration name argument as null, it will default to the configuration source location
...
It is unfortunately common that users modify components (appenders, filters, etc.) of the currently active configuration.
This can lead to problems as the ones in the example below:

[source,java,indent=0]
----
include::example$manual/customconfig/ModifyConfiguration1.java[tag=wrong]
----
<1> Retrieving the `LoggerContext` can throw an exception if a different implementation of Log4j API is used.
<2> To prevent loss of log events, the old appender should be removed **after** the new one has been installed.
<3> Appenders should be stopped **after** they have been removed.
<4> Appenders should be started **before** they have been added.

This approach is prone to several problems:

* Your code relies on Log4j Core internals, which have lesser compatibility guarantees than the configuration file format.
Major releases can break your code even if they don't break your configuration file.

* You move out from the safety zone, where Log4j Core takes care of component's' life cycle (initialization, reconfiguration, etc.), and step into a minefield seriously undermining the reliability of your logging setup.

If you happen to have code programmatically modifying components of a configuration,
you can easily transform it into a custom `Configuration` implementation:

[source,java,indent=0]
----
include::example$manual/customconfig/ModifyConfiguration2.java[tag=right]
----
<1> `Configurator` takes care of retrieving the correct `LoggerContext` and check it is of the expected type.
<2> The removal and addition of appenders can be performed in an arbitrary order.
<3> Appenders don't need to be stopped.
<4> Appenders don't need to be started.
