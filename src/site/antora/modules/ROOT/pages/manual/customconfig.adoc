...
If you have ever created a xref:manual/configuration.adoc[Log4j Core configuration file], consider `ConfigurationBuilder` as a convenience utility to model the very same declarative configuration structure programmatically, in a type-safe way.
...
This is a convenient way to create single-use, isolated ``LoggerContext``s for tests, etc.
...
*We strongly advise you to avoid programmatically modifying components of a configuration!*
