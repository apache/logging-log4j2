...
xref:manual/lookups.adoc[] (e.g., `${java:version}`, `${env:USER}`, `${date:MM-dd-yyyy}`) found in the `pattern` will be resolved.
...
<3> This works, even though it is located in a _configuration object of a resolver_, since <<event-template-resolver-pattern,the `pattern` resolver>> explicitly perform substitutions on the `pattern` argument.
