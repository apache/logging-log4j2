...
JSON Template Layout is a customizable, xref:#performance[efficient], and xref:#faq-garbage-free[garbage-free] JSON generating layout.
...
|===
|Type |`String`
|Default value |Refer to xref:#recycling-strategy[]
...
{project-github-url}/tree/2.x/log4j-layout-template-json/src/main/resources/JsonLayout.json[`JsonLayout.json`]::
...
The effective recycler factory can be configured using the xref:#plugin-attr-recyclerFactory[] plugin attribute.
...
xref:#plugin-attr-recyclerFactory[] layout configuration attribute is converted from `String` to a `RecyclerFactory` using the default `RecyclerFactoryConverter` extending from `TypeConverter<RecyclerFactory>`.
...
* The configured link:#recycling-strategy[recycling strategy] might not be garbage-free.
