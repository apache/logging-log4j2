...
Note that the value of this attribute in a configuration file will not be subject to <<faq-lookups,property substitution>>.
...
`key`:: Entry key
`value`:: Entry value.
This attribute in a configuration file will not be subject to <<faq-lookups,property substitution>>.
`format`::
Format of the value.
Accepted values are:
...
The **default event template** modelling https://www.elastic.co/guide/en/ecs/current/ecs-reference.html[the Elastic Common Schema (ECS) specification]
...
<<faq-lookups,Property substitutions>> found in the `pattern` will be resolved.
...
[#faq-lookups]
=== Is property substitution, including lookups, supported in templates?

Yes, xref:manual/configuration.adoc#property-substitution[property substitution] (e.g., `$\{myProperty}`), including xref:manual/lookups.adoc[lookups] (e.g., `${java:version}`, `${env:USER}`, `${date:MM-dd-yyyy}`), is supported in string literals in templates.
Though this doesn't apply to literals located in _configuration object of resolvers_, unless the associated resolver explicitly does so.
...
----
{
    ...
    "userId": {
      "$resolver": "mdc",
      "key": "${env:FOO:-userId}" //<2>
    },
    "pid": {
      "$resolver": "pattern",
      "pattern": "${env:NO_SUCH_KEY:-%pid}" //<3>
    },
    "instant": {
      "$resolver": "pattern",
      "pattern": "$${date:HH:mm:ss}" //<4>
    }
}
----
<1> This works. `${java:version}` will be replaced with the corresponding value _once_ while compiling the event template.
<2> This won't work! That is, `${env:FOO:-userId}` literal will not get substituted, since it is located in a _configuration object of a resolver_, and <<event-template-resolver-mdc,the `mdc` resolver>> does not explicitly do any substitutions either.
<3> This works, even though it is located in a _configuration object of a resolver_, since <<event-template-resolver-pattern,the `pattern` resolver>> explicitly performs substitutions on the `pattern` argument.
<4> This works.
As mentioned above, <<event-template-resolver-pattern,the `pattern` resolver>> explicitly performs substitutions on the `pattern` argument.
Hence, `$${date:HH:mm:ss}` will first be substituted with `${date:HH:mm:ss}` while compiling the event template, and then passed to the xref:manual/pattern-layout.adoc[] factory method.
Consequently, Pattern Layout will substitute `${date:HH:mm:ss}` with the value returned from xref:manual/lookups.adoc#DateLookup[Date Lookup] _for every log event(!)_.
+
Date Lookup is used as an example to demonstrate the case, *always prefer <<event-template-resolver-timestamp,the `timestamp` resolver>> over Date Lookup*.
...
*JSON Template Layout has powerful directives to inject external values, and you should always prefer those whenever possible over lookups!*
For example, always prefer <<event-template-resolver-map, the `map` resolver>> over xref:manual/lookups.adoc#MapLookup[the `map` lookup].
====

include::partial$manual/layouts-gcfree-property-substitution.adoc[]
...
Don't forget to check out <<event-template-resolvers,the notes on garbage footprint of resolvers>> you employ in templates.
