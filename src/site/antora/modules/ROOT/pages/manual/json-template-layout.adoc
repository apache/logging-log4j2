...
Keys listed in `disallowed` are silently dropped.
Both are matched against the key as found in the map, that is, before `replacement` is applied, and both can be combined with `pattern`.
