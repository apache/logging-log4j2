...
You are strongly advised to measure your application's overall performance and then, if Log4j is found to be an important bottleneck factor, tune it accordingly.
...
====
Remember that a logging API and a logging implementation are two different things.
You can use Log4j Core in combination with a logging API other than Log4j API (e.g., SLF4J).
