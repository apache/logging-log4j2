...
The database query (i.e., `db.findUserNameById(userId)`) can be a significant bottleneck if the created the log event will be discarded anyway – maybe the `INFO` level or the associated {log4j2-url}/manual/markers.adoc[marker] is not accepted for this package, or due to some other filtering.
...
----
+
While this would work for cases where the message can be dropped due to insufficient level, this approach is still prone to other filtering cases; e.g., maybe the associated {log4j2-url}/manual/markers.adoc[marker] is not accepted.
...
The xref:manual/appenders.adoc#AsyncAppender[asynchronous appender] accepts references to other appenders and causes log events to be written to them on a separate thread.
