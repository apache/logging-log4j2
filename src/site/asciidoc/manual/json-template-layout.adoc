...
certain number (`capacity`). When this limit is exceeded, it starts
allocating. `queue` is a good strategy where `threadLocal` is not desirable.
+
...
We ran the benchmark with the following settings:
