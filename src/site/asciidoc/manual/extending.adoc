...
`org.apache.logging.log4j.spi.LoggingSystemProvider`. Each implementation must
provide a class that implements `org.apache.logging.log4j.spi.LoggingSystemProvider` and
should have a no-arg constructor. Log4j will compare
...
..  Implement a class that implements link:../log4j-core/apidocs/org/apache/logging/spi/LoggingSystemProvider.html[`org.apache.logging.spi.LoggingSystemProvider`]
with a no-arg constructor.
..  Create a `META-INF/services/org.apache.logging.spi.LoggingSystemProvider` file
that contains the name of the class that implements
`org.apache.logging.spi.LoggingSystemProvider`.
..  Add `provides org.apache.logging.spi.LoggingSystemProvider with com.example.MyLoggingSystemProvider;` to `module-info.java`.
