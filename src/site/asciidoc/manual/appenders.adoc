...
|keys |String[] |The keys of the queue to which each log message should be sent.
...
|ssl |boolean |Whether or not to use ssl when connecting to Redis (default false).
...
<?xml version="1.0" encoding="UTF-8"?>
  ...
  <Appenders>
    <Redis name"RedisAppend" host="localhost" port=6379 ssl=true>
      <PatternLayout>
        <Pattern>%d %p %c{1.} [%t] %m%n</Pattern>
      </PatternLayout>
    </Redis>
