...
suffix KB, MB or GB, for example `20MB`. The file pattern must contain
a `%i` otherwise the target file will be overwritten on every rollover
as the SizeBased Triggering Policy does not cause the timestamp value
