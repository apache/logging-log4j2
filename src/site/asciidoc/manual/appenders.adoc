...
|host |String |The hostname on which Redis is running.

|keys |String |The keys of the queue to which each log message should be sent, separated by commas (e.g. "key1,key2,key3").
...
|immediateFlush | boolean | Whether to immediately send logs to redis rather than sending at the end of a batch.

|queueCapacity | int | The maximum number of logs to hold in memory before sending to Redis.

|port |int |The port on which Redis is running.
...
Here is a sample RedisAppender configuration snippet:
...
<?xml version="1.0" encoding="UTF-8"?>
  ...
  <Appenders>
    <Redis name"RedisAppend" host="localhost" port=6379 keys="key1,key2">
      <PatternLayout>
        <Pattern>%m</Pattern>
      </PatternLayout>
    </Redis>
...
http://logging.apache.org/log4j/2.x/manual/appenders.html#AsyncAppender[Async
appender] and/or set syncSend to `false` to log asynchronously (as above).
