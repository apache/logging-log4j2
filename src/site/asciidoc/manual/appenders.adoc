...
|keys |String |The keys of the queue to which each log message should be sent, separated by commas (e.g. "key1,key2,key3"). Default: "logEvents".
...
<?xml version="1.0" encoding="UTF-8"?>
  ...
  <Appenders>
    <Redis name"RedisAppend" host="localhost" port=6379 keys="key1,key2" immediateFlush=false>
      <PatternLayout>
        <Pattern>%m</Pattern>
      </PatternLayout>
    </Redis>
